...
def graph(x, y, c=0):
    g = []
    # north direction
    if 20 > x > 0 and y < 20 and draw[y, x - 1] == 0 and draw[y, x] == 0 and c_f_n[x, y + 1, 0] > c + 1:
        c_f_n[x, y + 1] = [c + 1, heuristic2(x, y + 1, x2, y2) + c + 1, x, y]
        n = [x, y + 1, c + 1, heuristic2(x, y + 1, x2, y2) + c + 1, x, y]
        g.append(n)
    elif 20 > x > 0 and y < 20 and (
    ...
    if x < 20 and y > 0 and draw[y - 1, x] == 0 and c_f_n[x + 1, y - 1][0] > c + 1.5:
        c_f_n[x + 1, y - 1] = [c + 1.5, heuristic2(x + 1, y - 1, x2, y2) + c + 1.5, x, y]
        se = [x + 1, y - 1, c + 1.5, heuristic2(x + 1, y - 1, x2, y2) + c + 1.5, x, y]
        g.append(se)
    # south direction
    if 0 < x < 20 and y > 0 and draw[y - 1, x - 1] == 0 and draw[y - 1, x] == 0 and c_f_n[x, y - 1][0] > c + 1:
        c_f_n[x, y - 1] = [c + 1, heuristic2(x, y - 1, x2, y2) + c + 1, x, y]
        s = [x, y - 1, c + 1, heuristic2(x, y - 1, x2, y2) + c + 1, x, y]
        g.append(s)
    elif 0 < x < 20 and y > 0 and (
    ...
    if x > 0 and y > 0 and draw[y - 1, x - 1] == 0 and c_f_n[x - 1, y - 1][0] > c + 1.5:
        ...
        g.append(sw)
    # west direction
    if 0 < y < 20 and x > 0 and draw[y - 1, x - 1] == 0 and draw[y, x - 1] == 0 and c_f_n[x - 1, y][0] > c + 1:
        c_f_n[x - 1, y] = [c + 1, heuristic2(x - 1, y, x2, y2) + c + 1, x, y]
        w = [x - 1, y, c + 1, heuristic2(x - 1, y, x2, y2) + c + 1, x, y]
        g.append(w)
    elif 0 < y < 20 and x > 0 and (
        ...
        g.append(w)
    # north west
    if x > 0 and y < 20 and draw[y, x - 1] == 0 and c_f_n[x - 1, y + 1][0] > c + 1.5:
        c_f_n[x - 1, y + 1] = [c + 1.5, heuristic2(x - 1, y + 1, x2, y2) + c + 1.5, x, y]
